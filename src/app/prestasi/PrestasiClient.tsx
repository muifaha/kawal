...
export default function PrestasiClient({ user, classes, initialPrestasiList, defaultTab = "list" }: PrestasiClientProps) {
  ...
  const handleEditPiagamFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const base64 = await compressImageFile(file, 1200, 0.75);
      setEditFotoPiagamBase64(base64);
      setEditFotoPiagamPreview(base64);
    } catch (err) {
      console.error("Kompresi foto piagam edit gagal:", err);
    }
  };

  const handleEditKegiatanFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const base64 = await compressImageFile(file, 1200, 0.75);
      setEditFotoKegiatanBase64(base64);
      setEditFotoKegiatanPreview(base64);
    } catch (err) {
      console.error("Kompresi foto kegiatan edit gagal:", err);
    }
  };
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-xl w-full p-5 space-y-4 shadow-2xl my-8">
            ...
            <form onSubmit={handleSaveEditSubmit} className="space-y-3 text-xs">
              ...
              {/* Upload & Preview Foto Piagam & Foto Diri/Kelompok */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl space-y-1.5">
                  <label className="block text-[11px] font-semibold text-slate-300 uppercase tracking-wider">
                    Foto Piagam / Sertifikat
                  </label>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleEditPiagamFileChange}
                    className="block w-full text-[10px] text-slate-400 file:mr-2 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-[10px] file:font-semibold file:bg-slate-900 file:text-amber-400 hover:file:bg-slate-800 cursor-pointer"
                  />
                  {editFotoPiagamPreview && (
                    <div className="relative mt-2 w-24 h-24 rounded-lg overflow-hidden border border-slate-700 group">
                      <img src={editFotoPiagamPreview} alt="Piagam Preview" className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => {
                          setEditFotoPiagamBase64(null);
                          setEditFotoPiagamPreview(null);
                        }}
                        className="absolute top-1 right-1 p-0.5 bg-rose-600/80 text-white rounded-full transition-all cursor-pointer"
                        title="Hapus Foto"
                      >
                        <X className="w-3 h-3" />
                      </button>
                ...
                <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl space-y-1.5">
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleEditKegiatanFileChange}
                    className="block w-full text-[10px] text-slate-400 file:mr-2 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-[10px] file:font-semibold file:bg-slate-900 file:text-amber-400 hover:file:bg-slate-800 cursor-pointer"
                  />
                  {editFotoKegiatanPreview && (
                    <div className="relative mt-2 w-24 h-24 rounded-lg overflow-hidden border border-slate-700 group">
                      <img src={editFotoKegiatanPreview} alt="Kegiatan Preview" className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => {
                          setEditFotoKegiatanBase64(null);
                          setEditFotoKegiatanPreview(null);
                        }}
                        className="absolute top-1 right-1 p-0.5 bg-rose-600/80 text-white rounded-full transition-all cursor-pointer"
                        title="Hapus Foto"
                      >
